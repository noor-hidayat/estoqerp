...
rfqRouter.get("/rfqs/:id/compare", async (req, res, next) => {
  ...
  try {
    ...
    // grand totals per supplier (include paymentTerm & deliveryLeadTime for compare table)
    const totals = quotations.map(q=> {
      ...
      return {
        ...
        total,
        status: q.status,
        paymentTerm: (q as any).paymentTerm ?? null,
        deliveryLeadTime: (q as any).deliveryLeadTime ?? null,
        quotationNo: (q as any).quotationNo ?? null,
