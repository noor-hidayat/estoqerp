...
async function chunkInsert(table:unknown, rows:any[], chunk=CHUNK){
  for(let i=0;i<rows.length;i+=chunk){
    const slice=rows.slice(i,i+chunk);
    // @ts-expect-error
    await db.insert(table).values(slice).onConflictDoNothing();
    if((i/chunk)%4===0||i+chunk>=rows.length) console.log(`    chunk ${i+slice.length}/${rows.length} (${Math.round((i+slice.length)/rows.length*100)}%)`);
  }
}
...
async function seedStockBalancesFast(){
  ...
  console.log(`\n[stock_balances] fast: cur=${cur}, need=${need}`);
  const whs = await db.select().from(warehouses);
  const its = await db.select().from(items);
  // Strategy: iterate wh*item with cycling dates 0..9 to get ~100k unique wh|item|date
  ...
  const rows:any[]=[];
  let added=0;
  let dateOffsets = Array.from({length:60},(_,i)=>i); // 60 hari variasi
  let seq=cur+1;
  outer: for(const dOff of dateOffsets){
    ...
    for(const wh of whs){
      for(const it of its){
        ...
        existing.add(k);
        const h = (wh.id.charCodeAt(0)+it.id.charCodeAt(0)+dOff)%1000;
        const opening = 20 + (h%900);
        const inQty = h%40;
        const outQty = h%35;
        rows.push({
          id: `stb_fast_${String(seq).padStart(6,"0")}_${randomSuffix(2)}`,
          ...
          createdAt: new Date(Date.now()-dOff*86400000),
          updatedAt: new Date(Date.now()-dOff*86400000),
        });
        seq++; added++;
        if(rows.length>=CHUNK){
          await db.insert(stockBalances).values(rows as any).onConflictDoNothing();
          console.log(`  stock_balances: ${added}/${need} (${Math.round(added/need*100)}%)`);
          rows.length=0;
        }
      }
    }
  }
  if(rows.length){ await db.insert(stockBalances).values(rows as any).onConflictDoNothing(); console.log(`  stock_balances final: ${added}/${need}`); }
  const final = Number((await db.execute(sql`SELECT count(*)::int n FROM stock_balances`) as any).rows[0].n);
  console.log(`  stock_balances final count ${final}`);
}
...
async function seedStockBatchesFast(){
  ...
  console.log(`\n[stock_batches] fast: cur=${cur}, need=${need}`);
  const whs = await db.select().from(warehouses);
  ...
  for(let i=0;i<need;){
    ...
    const wh=pick(whs);
    const k=`${b.id}|${wh.id}`;
    if(existing.has(k)) { if(i>need*5) break; continue; }
    existing.add(k);
    rows.push({ id: genId("stbch", randomDate()), batchId: b.id, warehouseId: wh.id, qty: String(randomInt(0,500)), updatedAt: randomDate() });
    added++; i++;
    if(rows.length>=CHUNK){
      await db.insert(stockBatches).values(rows).onConflictDoNothing();
      console.log(`  stock_batches: ${added}/${need} (${Math.round(added/need*100)}%)`);
      rows.length=0;
    }
  }
  if(rows.length){ await db.insert(stockBatches).values(rows).onConflictDoNothing(); }
  const final = Number((await db.execute(sql`SELECT count(*)::int n FROM stock_batches`) as any).rows[0].n);
  console.log(`  stock_batches final ${final}`);
}
...
async function seedOpnameFast(){
  ...
  if(needP>0){
    ...
    for(let off=0; off<needP; off+=CHUNK){
      ...
      await chunkInsert(opnameProjects, rows, 1000);
  ...
  if(needOw>0){
    ...
    while(added<needOw){
      ...
      await db.insert(opnameWarehouses).values(rows).onConflictDoNothing();
      added+=rows.length;
  ...
  if(needS>0){
    ...
    for(let off=0; off<needS; off+=CHUNK){
      ...
      await chunkInsert(opnameScans, rows, 1000);
  ...
  if(needSd>0){
    ...
    for(let off=0; off<needSd; off+=CHUNK){
      ...
      await chunkInsert(opnameScanDetails, rows, 1000);
  ...
  if(needOc>0){
    ...
    for(let off=0; off<needOc; off+=CHUNK){
      ...
      await chunkInsert(opnameCounts, rows, 1000);
  ...
  if(needOcd>0){
    ...
    for(let off=0; off<needOcd; off+=CHUNK){
      ...
      await chunkInsert(opnameCountDetails, rows, 1000);
...
async function main(){
  console.log(`=== Resume 100k: stock_balances, stock_batches, opname ===`);
  await seedStockBalancesFast();
  await seedStockBatchesFast();
  await seedOpnameFast();
  console.log("\n=== Final counts ===");
  for(const t of ["stock_balances","stock_batches","opname_projects","opname_warehouses","opname_scans","opname_scan_details","opname_counts","opname_count_details","branches","warehouses","users"]){
