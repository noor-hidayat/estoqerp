...
import { TEMPLATE_BY_ID } from "@/modules/dashboard/registry/widget-registry";
import {
  dimLabel,
  type WidgetConfig,
  type WidgetInstance,
...
function BarWidget({ widget, dragHandle, data: presetData, isLoading: presetLoading }: WidgetProps) {
  ...
  const { data: dynamicDataBar, isLoading: dynamicLoadingBar } = useWidgetQuery(
    isReceivingBar ? (dynamicConfigBar as unknown as import("@/modules/dashboard/types").WidgetConfig) ?? undefined : undefined
  );
  const { data: queryData, isLoading: queryLoading } = useWidgetQuery(
...
function LineWidget({ widget, dragHandle, data: presetData, isLoading: presetLoading }: WidgetProps) {
  ...
  const { data: dynamicData, isLoading: dynamicLoading } = useWidgetQuery(
    isMultiPurchaseDelivery ? (dynamicConfig as unknown as import("@/modules/dashboard/types").WidgetConfig) ?? undefined : undefined
  );
  const { data: dynamicDataStock, isLoading: dynamicLoadingStock } = useWidgetQuery(
    isStockLevelFg ? (dynamicConfigStock as unknown as import("@/modules/dashboard/types").WidgetConfig) ?? undefined : undefined
  );
