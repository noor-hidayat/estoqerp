...
export default function EditWarehousePage() {
  ...
  const save = async () => {
    if (!form.code.trim() || !form.name.trim() || !form.branchId) {
      setError("Code, warehouse name, and plant are required.");
      return;
    }
    if (
      warehouses.some(
    ...
    return (
      <RoleGuard roles={MANAGER_ROLES} menus={["inventory.warehouses"]}>
        ...
        <div className="text-center">
          <Link href="/app/stock/warehouses" className="text-sm text-primary hover:text-primary/80">Back to Warehouses</Link>
        </div>
      </RoleGuard>
    );
  }
  ...
  return (
    <RoleGuard roles={MANAGER_ROLES} menus={["inventory.warehouses"]}>
      ...
      >
        ...
        <FormActions>
          <Button variant="ghost" onClick={() => navigate("/app/stock/warehouses")}>
