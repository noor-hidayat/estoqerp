...
import type { BarcodeFormat, Item, ScanRecord, ScanSession } from "@/types";
...
export default function ScanSessionPage() {
  ...
  // Sumber kebenaran sesi aktif: query refetch bisa tertinggal tepat setelah
  // sesi dibuat/ditutup — ref ini di-set sinkron agar record scan tidak pernah
  // tersimpan tanpa sessionId/locationId.
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-zinc-200 bg-white py-20 text-center">
        <ScanLine size={28} strokeWidth={2} className="text-zinc-300" />
  ...
  const startSession = async (): Promise<ScanSession | null> => {
    ...
    try {
      ...
      setFeed({
        ...
        detail: e instanceof Error ? e.message : "Coba lagi.",
      });
      return null;
    }
  };
  ...
  const recordScans = async (
    ...
    if (!session) {
      throw new Error("Sesi scan belum aktif. Scan barcode sekali untuk memulai sesi.");
    }
  ...
  const handleScan = async (
    ...
    if (!locationId) {
      setFeed({
        ...
        detail: "Wajib memilih lokasi sebelum mulai scan.",
      });
      return;
    }
  ...
  const confirmSave = async () => {
    ...
    if (needsManual && (!Number.isFinite(manual) || manual < 1)) {
      setFeed({
        ...
        detail: "Isi jumlah qty (harus lebih dari 0).",
      });
      return;
    }
    ...
    if (!session) {
      setFeed({
        ...
        detail: "Scan barcode sekali untuk memulai sesi baru, lalu simpan lagi.",
      });
      return;
    }
    ...
    try {
      ...
      setFeed({
        ok: false,
        title: "Gagal menyimpan scan",
        detail: `${
          e instanceof Error ? e.message : "Coba lagi."
        } Sebagian scan mungkin sudah tersimpan — periksa daftar "Scan terakhir" sebelum mencoba lagi.`,
      });
      setSaving(false);
      return;
    }
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="min-w-0 flex-1 sm:max-w-xs">
              <ComboBox
                ...
                placeholder="Pilih lokasi rak/bin..."
            ...
            <div className="h-[308px] overflow-y-auto">
                  {history.map((h) => (
                    <div key={h.key} className="flex h-11 items-center px-5">
                      <p className="min-w-0 flex-1 truncate font-mono text-[13px] tracking-wider text-zinc-900">
                        {h.barcode}
                      </p>
                    </div>
                  ))}
                ...
                <div className="border-b border-zinc-100 px-5 py-4">
                  ...
                  <p className="mt-0.5 text-[12px] text-zinc-500">
                    {buffer.some((e) => !e.format.qtyPerFormat)
                      ? "Isi qty yang sama untuk semua barcode, lalu simpan ke sesi."
                      : "Qty otomatis dari master item per barcode."}
                  </p>
                </div>
                ...
                {buffer.some((e) => !e.format.qtyPerFormat) && (
                  <div className="border-b border-zinc-100 px-5 py-4">
                    ...
                    <input
                      ...
                      className="h-10 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3.5 text-center font-mono text-sm text-zinc-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                    />
                  </div>
                )}
                      ...
                      <div key={e.barcode} className="flex items-center gap-3 px-5 py-3">
                        ...
                        <div className="text-right">
                          ...
                          {e.format.qtyPerFormat ? (
                            ...
                            <span className="block font-mono text-[13px] font-semibold text-zinc-900">
                              {manualQty.trim() === ""
                                ? "—"
      ...
      <AnimatePresence>
        {feed && !feed.ok && (
          <motion.div
            ...
            className="fixed right-4 top-20 z-[60] flex w-80 max-w-[calc(100vw-2rem)] items-start gap-3 rounded-lg border border-red-200 bg-white p-4 shadow-[0_8px_24px_-8px_rgb(24_24_27/0.15)]"
          >
