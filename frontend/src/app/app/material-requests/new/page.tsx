...
export default function NewMaterialRequestPage() {
  ...
  const { user } = useSession();  const { data: warehouses = [], isLoading: warehousesLoading } = useAllWarehouses();
  ...
  const [form, setForm] = useState({    warehouseId: "",
  ...
  // Need Approval default true if hasDefaultMR workflow for PR
  useEffect(() => {
    if (needApprovalManuallySet) return;
    const should = hasDefaultMR;
    setForm((f) => (f.needApproval === should ? f : { ...f, needApproval: should }));
  }, [hasDefaultMR, needApprovalManuallySet]);
  ...
  const submit = async () => {
    ...
    try {
      const res = await create.mutateAsync({        warehouseId: form.warehouseId || warehouses[0]?.id || null,
      ...
      navigate(`/app/material-requests/${(res as any).id}`);
    } catch (e) {
      ...
      <FormPage
        ...
        actions={
          <div className="flex items-center gap-2">
            <DocMenu onCancel={() => navigate("/app/material-requests")} onDelete={resetForm} />
