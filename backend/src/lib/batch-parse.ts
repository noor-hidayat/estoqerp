...
// Logika ini disalin ke frontend/src/lib/batch/parser.ts (sinkron manual).

export const BATCH_SEGMENT_FIELDS = ["DATE", "SHIFT", "SEQUENCE", "ALTERNATIVE_CODE", "CUSTOM"] as const;
export type BatchSegmentField = (typeof BATCH_SEGMENT_FIELDS)[number];

export const BATCH_SEGMENT_MODES = ["POSITION", "DELIMITER"] as const;
export type BatchSegmentMode = (typeof BATCH_SEGMENT_MODES)[number];

export const BATCH_DATE_FORMATS = ["YYMMDD", "DDMMYY", "YYYYMMDD", "YYYY-MM-DD"] as const;
export type BatchDateFormat = (typeof BATCH_DATE_FORMATS)[number];

export interface BatchSegment {
  id: string;
  field: BatchSegmentField;
  mode: BatchSegmentMode;
  /** Mode POSITION: start/end (1-based, inklusif). */
  start?: number;
  end?: number;
  /** Mode DELIMITER: delimiter string + index segmen (0-based). */
  delimiter?: string;
  index?: number;
  /** Field DATE: format tanggal yang dipakai. */
  dateFormat?: BatchDateFormat;
  /** Field CUSTOM: label custom field. */
  label?: string;
}

export interface BatchFormatLike {
  id: string;
  name: string;
  isActive: boolean;
...
export interface BatchParseResult {
  ...
  shift: string | null;
  /** Kode alternatif item yang ter-encode di batch number (field ALTERNATIVE_CODE). */
